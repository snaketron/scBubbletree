...
Load the processed PC matrix and the meta data
...
with e.g. the Louvain method, then we need to find appropriate value for the 
resolution parameter $r$. Otherwise, we can use the simpler k-means clustering 
algorithm in which case we need to find an appropriate value of the number of 
clusters $k$.

### 1.1 determining the resolution parameter $r$ for Louvain clustering
...
`get_r` for data-driven inference of $r$ based on the Gap statistic and the 
...
a vector of $r$s to inspect. Here the $r$s are:

```{r, eval = FALSE}
10^seq(from = -4, to = 0.5, by = 0.5)
```


```{r, include = FALSE}
knitr::kable(10^seq(from = -4, to = 0.5, by = 0.5), col.names = "r")
```

For more/less detailed analysis, you can should decrease/increase the 
increment number (`by`). See the help function `?get_r` to learn more 
...
($r=0.003$). Means (points) and 95% confidence intervals are shown for the 
...
We will use $r=0.003$ for clustering.

```{r, fig.width=4, fig.height=3, fig.align='center'}
ggplot(data = b_r$gap_stats_summary)+
    geom_point(aes(x = r, y = k))+
    xlab(label = "r")+
    ylab(label = "k'")+
  scale_x_log10()+
  annotation_logticks(base = 10, sides = "b")
...
### 1.2 [alternative] determining the number of clusters $k$ for k-means clustering
If we want to use k-means for clustering, then we need to find a reasonable 
value of $k$, e.g. by applying a data-driven search for $k$ using `get_k`.

Here `get_k` will inspect the Gap and WCSS at $k$ = 1, 2, ..., 20.
...
For simple scRNA-seq datasets, such as the one that is currently analyzed, 
we anticipate similar results from k-means and GCD methods. Hence, in the 
Gap statistic and WCSS curves shown below we notice a knee (elbow) at $k=5$ 
$\rightarrow$ comparable results as in 1.1. For complex scRNA-seq datasets 
GCD methods are recommended.
...
### 2.1 with Louvain
Now that we found out that $r=0.003$ ($k'=5$) is a reasonable choice based on 
the data, we will perform Louvain clustering with $r=0.003$ and $A$ as inputs. 
For this we will use the function `get_bubbletree_graph`.

There are multiple variants of the Louvain algorithm (see function 
`FindClusters`, R-package `r CRANpkg("Seurat")`). Here we will use the 
original implementation (`algorithm` = "original"). 

We will do clustering with $n_\textit{start}=20$ random starts and a maximum 
number of iterations $iter_\textit{max} = 100$. For input, we construct a SNN 
graph based on $A^{3,918\times 15}$, where each vertex (cell) was connected to 
its $knn_{k}=50$ nearest neighbors. See the help `?get_bubbletree_graph` to 
learn about the remaining parameters.
...
After clustering, `r Biocpkg("scBubbletree")` organizes the bubbles as leaves 
of a hierarchical dendrogram (=bubbletree). The distance between clusters 
$i$ and $j$ can be computed by estimating the average Euclidean distance 
between all pairs of cells in $C_{i}^{n_i\times f}$ and $C_{j}^{n_j\times f}$. 
However, this operation has considerable memory and computational cost. To 
avoid such high memory demands, `r Biocpkg("scBubbletree")` employs a 
bootstrapping approach with $B$ iterations

In iteration $b$, the algorithm draws with replacement a random subset 
with $n'_i=\min(n_i, N_{\textit{eff}})$ and $n'_j=\min(n_j, N_{\textit{eff}})$ 
number of cells from cluster $i$ and $j$, respectively, where $N_{\textit{eff}}$
is a user-defined parameter that controls the maximum number of cells to be 
drawn from each cluster. The algorithm computes the average distance between 
cluster $i$ and $j$ based on $n'_in'_j$ inter-cell Euclidean distances, and 
stores the result in distance matrix $D^{k\times k}_b$: 
\begin{equation}
	D_{b}^{ij} = \dfrac{1}{n'_in'_j}\sum\limits_{\substack{\forall 
	\mathbf{x}\in X\\ \forall \mathbf{y}\in Y}}\sqrt{(\mathbf{x}-\mathbf{y})^2},
\end{equation}
where $X^{n'_i \times f}$ and $Y^{n'_j \times f}$ are subsets drawn from 
$C_{i}^{n_i\times f}$ and $C_{j}^{n_j\times f}$, respectively, and 
$\mathbf{x}$ and $\mathbf{y}$ are cell transcriptome features (row vectors) 
from each matrix. At the end of bootstrap iteration $b$, the distance matrix 
$D^{k'\times k'}_{b}$ is provided as input for hierarchical clustering with 
average linkage to generate the dendrogram $H_b$. 

From the collection of $B$ distance matrices we compute an average distance 
matrix ($\hat{D}^{k'\times k'}$), and use $\hat{D}^{k'\times k'}$ to generate a 
consensus hierarchical dendrogram (bubbletree; $\hat{H}$) by hierarchical 
clustering with average linkage. The collection of bootstrap dendrograms are 
used to assess the robustness of the bubbletree topology by quantifying the 
number of times each branch in $\hat{H}$ was found among the topologies of 
the bootstrap dendrograms. Branches can have variable degrees of support 
ranging between values close to 0 (no support) and $B$ (complete support). 
Distances between individual bubbles are visualized quantitatively as sums 
of branch lengths in root-to-leaf direction between these bubbles.
...
The two dendrograms are shown side-by-side. We see nearly identical dendrograms. 
The bubble IDs are different but we see similar bubble sizes, topology and 
branch robustness values.
...
heatmap are color coded according to the jaccard distances between the pairs 
of bubbles from the two bubbletrees, and the tile labels show the numbers 
of cells in common between the bubbles.

**Reminder of the Jaccard index ($J$) and the Jaccard distance ($J_{D}$)**: 
For clusters $A$ and $B$ $J(A,B)=\dfrac{|A \cap B|}{|A \cup B|}$ and 
$J_{D}(A,B) = 1-J(A,B)$. If $A$ and $B$ contain the same set of cells 
$J_{D}(A,B)$=0, and if they have no cells in common $J_{D}(A,B)$=1.
