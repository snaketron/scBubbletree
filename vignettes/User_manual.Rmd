...
at $k'=5$ ($r=0.003$). Means (points) and 95% confidence intervals are shown 
...
#### Comparison of bubbletree pairs
To compare a pair of bubbletrees and their clustering solutions we can use 
the function `compare_bubbletrees`. The function produces the following
output:

1. heatmap with jaccard distances (JDs) between the pairs of bubbles from the two 
bubbletrees (JD=0 if both clusters contain the same set of cells; JD=1 for 
completely disjoint clusters). The tile labels in the heatmap show the numbers 
of intersecting cells between the bubbles of the two bubbletrees. 

From the heatmap we nearly identical clusterings between the bubbletrees. 
Only 3 cells (in tiles with label = 1) are classified differently by the
two bubbletrees.

```{r, fig.width=8, fig.height=8}
cp <- compare_bubbletrees(btd_1 = k5_kmeans, 
                          btd_2 = k5_louvain,
                          ratio_heatmap = 0.6,
                          tile_bw = F,
                          tile_text_size = 3)
