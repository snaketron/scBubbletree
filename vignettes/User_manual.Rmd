...
After the clustering is complete we will organize the bubbles as leaves of
a hierarchical dendrogram (=bubbletree). For this, we will first compute 
inter-cluster distances (e.g. euclidean or Manhattan) between the centroids 
of each clusters computed from $A^{n\times f}$.  Distances between individual 
bubbles are visualized quantitatively as sums of branch lengths in root-to-leaf 
direction between these bubbles.

To assess the robustness of the bubbletree topology, `r Biocpkg("scBubbletree")`
can perform $B$ bootstrap iterations (e.g. $B=200$). In iteration $b$, we 
draw a random subset of $N_\textit{eff}$ (default $N_\textit{eff}=200$) cells 
with replacement from each cluster. Next, for each pair of cells in cluster $i$
and $j$ we compute their distance. The distance metric is defined by the user.
From the inter-cell distances we estimate the average inter-bubble distance, 
and we use this data to populate the distance matrix ($D^{k\times k}_b$), which 
is then provided as input for hierarchical clustering with an user-specified 
linkage method to generate a hierarchical clustering dendrogram $H_b$. 

The collection of dendrograms ($H_b$) are used to quantify the robustness 
of the bubbletree topology, i.e. to count the number of times each branch in 
the bubbletree is found among the topologies of the bootstrap dendrograms.
Branches can have has variable degrees of support ranging between close to 0 
(no support) and $B$ (complete support). 
