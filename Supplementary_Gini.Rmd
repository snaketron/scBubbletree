...
get_clusters_graph2 <- function(d, res, reduction_type, dims, labels) {
...
```{r}
leuven_umap <- mclapply(X = 10^seq(from = -4, to = 1, by = 0.1),
...
```{r}
leuven_tsne <- mclapply(X = 10^seq(from = -4, to = 1, by = 0.1),
                        FUN = get_clusters_graph, 
                        d = d,
                        labels = d@meta.data$celltype.l2,
                        dims = 1:2, 
...
```{r}
leuven_pca <- mclapply(X = 10^seq(from = -4, to = 1, by = 0.1),
                       FUN = get_clusters_graph, 
                       d = d,
                       labels = d@meta.data$celltype.l2,
                       dims = 1:15, 
...
```{r, fig.width=5, fig.height=3.75}
i <- gini_A[gini_A$reduction_type %in% c("kmeans", "pca"), ]
i$reduction_type[i$reduction_type == "pca"] <- "Louvain"
i$reduction_type[i$reduction_type == "kmeans"] <- "k-means"

g_a2 <- ggplot(data = i)+
  geom_line(aes(x = k, y = gini, col = reduction_type), 
            size = 0.8, alpha = 0.8)+
  ylab(label = "Gini impurity (cell line)")+
  geom_point(aes(x = k, y = gini, col = reduction_type), size = 1)+
  xlim(c(0, 50))+
  theme_bw(base_size = 10)+
  theme(legend.position = "top")+
  scale_color_grey(name = "algorithm")
...
i <- gini_B[gini_B$reduction_type %in% c("kmeans", "pca"), ]
i$reduction_type[i$reduction_type == "pca"] <- "Louvain"
i$reduction_type[i$reduction_type == "kmeans"] <- "k-means"
