...
g0 <- ggplot(data = b$gap_stats_summary)+
  ...
  ylab(label = "Gap")|
ggplot(data = b$wcss_stats_summary)+
  geom_point(aes(x = k, y = wcss_mean), size = 0.5)+
  geom_errorbar(aes(x = k, y = wcss_mean, ymin = L95, ymax = H95), width = 0.2)+
  ylab(label = "WCSS")+
  scale_y_log10()+
  annotation_logticks(base = 10, sides = "l")


fig_2_bottom <- (k20$tree|w2$w|w0$w)+patchwork::plot_layout(widths = c(2, 3, 3))
fig_2_top <- g0
...
sup_w <- (get_num_feature_violins(d = k20,
                                 as = data.frame(MT = a$mt_prop),
                                 plot_title = "",
                                 show_cells = F)$w|
            get_num_feature_violins(d = k20,
...
```{r, fig.width=8, fig.height=12, fig.align='center', echo=T}
...
```{r}
g_sup <- (((k20$tree|w3$w)+patchwork::plot_layout(widths = c(1, 1.3)))/w5$w)+
...
g <- (g0|g_gini)/((k20$tree|w1$w)+patchwork::plot_layout(widths = c(1.5,6)))+
...
umap_data$k <- k20$cluster
tsne_data$k <- k20$cluster
