...
is <- sample(x = 1:nrow(p@meta.data), size = 10000, replace = F)
...
markers_RNA <- as.matrix(t(d@assays$SCT@data[
  rownames(d@assays$SCT@data) %in% 
    c("IL7R", 
      ...
      "FCER1A", "CST3",
      "PPBP"), ]))
save(markers_RNA, file = "case_study_D/markers_RNA.RData")

markers_ADT <- as.matrix(t(d@assays$ADT@data))
save(markers_ADT, file = "case_study_D/markers_ADT.RData")
```



```{r}
umap <- cbind(meta, d@reductions$umap@cell.embeddings)
tsne <- cbind(meta, d@reductions$tsne@cell.embeddings)


save(umap, file = "case_study_D/umap.RData")
save(tsne, file = "case_study_D/tsne.RData")
```
