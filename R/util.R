...
#' Wrapper function around k-means. Input: features matrix \code{x} with cells 
#' as rows and features (such as PC embeddings) as columns; vector \code{ks} for
#' the number of k-means to try; B=number of bootstrapping iterations; the 
...
get_kmeans_boot <- function(B = 20, 
  ...
  get_avg_sil <- function(km, df, only_avg) {
    ss <- silhouette(km$cluster, dist(x = df, method = "euclidean"))
    if(only_avg) {
      if(is.na(ss)) {
        return(NA)
      } 
  ...
  for(b in 1:B) {
    ...
    # extract silhouette
    cat("2) silhouette analysis done, ")
    sil_kmeans <- lapply(X = kmeans_obj, FUN = get_avg_sil, 
                         df = x[j,], only_avg = T)
    
    # within sum of squares
    cat("3) WSS analysis done. \n")
  ...
  # collect boot silhouette data
  silhouette_stats <- c()
  for(i in 1:length(boot_obj)) {
    silhouette_stats <- rbind(silhouette_stats, data.frame(boot_obj[[i]]$sil))
  }
  
  silhouette_stats <- reshape2::melt(data = silhouette_stats)
  silhouette_stats$boot <- as.numeric(gsub(pattern = "X", 
                                           replacement = '', 
                                           x = silhouette_stats$variable))
  silhouette_stats$variable <- NULL
  
  
  # collect WSS data
  wss_stats <- c()
  for(i in 1:length(boot_obj)) {
    wss_stats <- rbind(wss_stats, data.frame(boot_obj[[i]]$wss))
  }
  
  wss_stats <- reshape2::melt(data = wss_stats)
  wss_stats$boot <- as.numeric(gsub(pattern = "X", 
                                    replacement = '', 
                                    x = wss_stats$variable))
  wss_stats$variable <- NULL
...
get_bubble_tree <- function(btd, 
  ...
  tree <- ggtree::ggtree(ph, linetype='solid')%<+%km_meta+
    geom_point()+
    layout_rectangular()+
    geom_tippoint(aes(size = n, fill = n), shape = 21)+
    geom_tiplab(aes(label=paste0(label, " (", n, ', ', pct, "%)")), 
