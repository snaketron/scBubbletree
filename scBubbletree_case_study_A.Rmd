...
predicted cell types across the bubbles (parameter *feature_composition=T*).
...
parameter *feature_composition=F*).
...
(k5$tree|w3$w|w4$w)+patchwork::plot_layout(widths = c(1.5, 2.2, 2.5))
```
...
ggplot(data = b$wss_stats_summary)+
  ...
  scale_y_log10()+
  annotation_logticks(base = 10, sides = "l"))+
  patchwork::plot_layout(widths = c(1, 0.7, 0.7, 0.7))
```
...
g <- ((g0)/
        ((k5$tree|w_cell_line$w)+patchwork::plot_layout(widths = c(1,2)))/
        ((w_expression_avg$w|w4$w)+patchwork::plot_layout(widths = c(1,1))))+
  patchwork::plot_layout(heights = c(1, 2, 1.5))+
...
(u_k5$tree|u_w1$w)+patchwork::plot_layout(ncol = 2, widths = c(1, 1))
```
