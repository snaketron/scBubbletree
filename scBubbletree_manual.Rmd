...
scBubbletree also implement the function `get_gini_boot`, which allows us to 
...
g <- ((g0)/
        ...
        ((w3$w|w4$w|g1)+patchwork::plot_layout(widths = c(1.2, 3, 1.2))))+
  patchwork::plot_layout(heights = c(1, 2.5, 1))+
  patchwork::plot_annotation(tag_levels = 'A')
g

ggsave(plot = g,
       filename = "manuscript_data/fig_a.svg",
       device = "svg",
       width = 7,
       height = 8)
...
In essence, we will perform the same steps as in *Case study A*:
  ...
  `update_bubbletree`, `get_gini`, `get_gini_boot`
...
g_umap
ggsave(plot = g_umap, 
       filename = "manuscript_data/UMAP_B.pdf",
       device = "pdf",
       width = 6,
       height = 6)
```
...
`get_gini_boot`. One potential caveat of this approach is that some of the 
predictions might be inaccurate. 

Lets invoke the function `get_gini_boot` and pass the object `b` obtained 
...
```{r, fig.width=10, fig.height=14, fig.align='center', echo=T}
...
g <- (g0|g_gini)/((k15$tree|w1$w)+patchwork::plot_layout(widths = c(1,6)))+
...
ggsave(plot = g,
       filename = "manuscript_data/fig_b.svg",
       device = "svg",
       width = 11,
       height = 8.5)
...
ggsave(plot = w3$w,
       filename = "manuscript_data/fig_c.svg",
       device = "svg",
       width = 3,
       height = 6)

ggsave(plot = w4$w,
       filename = "manuscript_data/fig_d.svg",
       device = "svg",
       width = 6,
       height = 6)
