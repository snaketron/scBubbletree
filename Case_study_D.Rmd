---
title: "Case study D: digit recognition with scBubbletree"
author: "SK"
output:
...
# if we want we can apply PCA 
#
# is <- which(apply(X = digits, MARGIN = 2, FUN = sum) == 0)
...
# A_summary <- summary(A)
# 
# plot(A_summary$importance["Cumulative Proportion",], 
...
As expected, the Gap statistic and WCSS curves appear to have at $k\approx10$. 
Lets use a value of $k$ slightly larger than 10, e.g. $k=15$ for clustering. 
Means and 95% confidence intervals are shown for the metrics (panels) at each 
$k$.
...
integrate to 100%). We will use a black and white color scale by setting 
`tile_bw=T`.
...
Local cell distances appear to be adequately recovered, long-range distances 
